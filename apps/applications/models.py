...
#     result = ForeignKey(Result,on_delete=CASCADE,related_name='images')
#     image = ImageField(upload_to='results/%Y/%m/%d' ,validators=[FileExtensionValidator(['jpg', 'jpeg', 'png','webp'])],null=True, blank=True)
#
#     def save(self, *args, **kwargs):
#         if self.image and not self.image.name.lower().endswith(".webp"):
#             img = PilImage.open(self.image)
...
#             img.save(buffer, format="WEBP", quality=80)
#
#             file_name = os.path.splitext(self.image.name)[0] + ".webp"
